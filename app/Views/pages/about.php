<!-- HERO -->
<section class="hero">
    <div class="hero-bg bg-about"></div>
    <div class="hero-content">
        <h1>We Manufacture<br>We Rebuild<br>We <span class="accent">Engineer</span></h1>
    </div>
</section>

<!-- PROGRESS SECTION -->
<section class="section">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:60px;align-items:start;">
        <div data-aos="fade-right">
            <div class="icon-text">
                <div class="icon-text-icon">⚙️</div>
                <div>
                    <h2 style="font-size:clamp(18px,2.2vw,26px);margin-bottom:12px;">We believe progress should be enabled with confidence — <span style="color:var(--orange);">not forced choices!</span></h2>
                    <p style="font-size:14px;color:var(--text-muted);line-height:1.8;">In heavy industries where machines power mines, ports, and infrastructure, decisions carry weight. Downtime is costly, compromises are risky, and reliability is non-negotiable. Yet too often, operators are forced between long delays and uncertain alternatives.</p>
                    <p style="font-size:14px;color:var(--orange);font-weight:700;margin-top:8px;">We exists to remove that dilemma.</p>
                </div>
            </div>
        </div>
        <div data-aos="fade-left">
            <div class="icon-text">
                <div class="icon-text-icon">🔧</div>
                <div>
                    <h2 style="font-size:clamp(18px,2.2vw,26px);margin-bottom:12px;">We do this by taking engineering <span style="color:var(--orange);">responsibility end to end.</span></h2>
                    <p style="font-size:14px;color:var(--text-muted);line-height:1.8;">At BSAS, we work at the intersection of engineering insight, manufacturing capability, and real-world repair experience. From teardown to rebuild, from testing to validation, we stand accountable for lifecycle performance.</p>
                    <p style="font-size:14px;color:var(--text-muted);line-height:1.8;margin-top:8px;">We don't trade in shortcuts. <span style="color:var(--orange);font-weight:700;">We restore balance by enabling informed engineering decisions backed by validation and accountability.</span></p>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- MARQUEE -->
<div class="marquee-wrap">
    <div class="marquee-track" aria-hidden="true">
        <span class="marquee-item">Engineering-led solutions company built to take responsibility for outcomes<span class="dot">·</span></span>
        <span class="marquee-item">One engineering brain. Complete responsibility<span class="dot">·</span></span>
        <span class="marquee-item">Validated. Verified. Accountable<span class="dot">·</span></span>
        <span class="marquee-item">Engineering-led solutions company built to take responsibility for outcomes<span class="dot">·</span></span>
        <span class="marquee-item">One engineering brain. Complete responsibility<span class="dot">·</span></span>
        <span class="marquee-item">Validated. Verified. Accountable<span class="dot">·</span></span>
    </div>
</div>

<!-- CAPABILITIES -->
<section class="section section-grey">
    <div class="section-label" data-aos="fade-up">What We Cover</div>
    <h2 class="section-title mb-8" data-aos="fade-up">Our Capabilities <span class="accent">Span</span></h2>
    <p class="section-sub" data-aos="fade-up">From Component to Complete Lifecycle</p>
    <div class="caps-grid">
        <div class="cap-card" data-aos="fade-up">
            <div class="cap-card-bg bg-workshop"></div>
            <div class="cap-card-overlay"></div>
            <div class="cap-card-content"><h4>Engine rebuild and test-led validation</h4></div>
        </div>
        <div class="cap-card" data-aos="fade-up" data-aos-delay="100">
            <div class="cap-card-bg bg-equip"></div>
            <div class="cap-card-overlay"></div>
            <div class="cap-card-content"><h4>Transmission, axle, driveline rebuilding</h4></div>
        </div>
        <div class="cap-card" data-aos="fade-up" data-aos-delay="200">
            <div class="cap-card-bg bg-parts"></div>
            <div class="cap-card-overlay"></div>
            <div class="cap-card-content"><h4>Hydraulic systems repair and calibration</h4></div>
        </div>
        <div class="cap-card" data-aos="fade-up" data-aos-delay="300">
            <div class="cap-card-bg bg-spare"></div>
            <div class="cap-card-overlay"></div>
            <div class="cap-card-content"><h4>Engineered spare parts solution</h4></div>
        </div>
    </div>
</section>

<!-- SAFETY FIRST -->
<div class="safety-dark" data-aos="fade-in">
    <div>
        <h2>Safety <span style="color:var(--orange);">First</span></h2>
        <p>At BSAS, safety isn't a policy — it's an engineering requirement. Every component we design, every part &amp; Equipment we manufacture, and every machine we rebuild is engineered with one non-negotiable principle: <strong style="color:#fff;">it must perform safely under load, under pressure, and under the harshest conditions.</strong></p>
    </div>
    <div class="safety-img bg-workshop"></div>
</div>

<section class="section">
    <div class="safety-items">
        <div class="safety-item" data-aos="zoom-in">
            <div class="safety-item-icon">🦺</div>
            <h4>Personal Protective Equipment</h4>
        </div>
        <div class="safety-item" data-aos="zoom-in" data-aos-delay="150">
            <div class="safety-item-icon">📋</div>
            <h4>Safety Instruction Manuals</h4>
        </div>
        <div class="safety-item" data-aos="zoom-in" data-aos-delay="300">
            <div class="safety-item-icon">🧠</div>
            <h4>Physical &amp; Psychological Safety</h4>
        </div>
    </div>
</section>

<!-- MISSION & VISION -->
<section class="section section-dark">
    <div class="section-label" data-aos="fade-up">Our Purpose</div>
    <h2 class="section-title mb-40" data-aos="fade-up">Mission &amp; <span class="accent">Vision</span></h2>
    <div class="mv-grid">
        <div class="mv-card" data-aos="fade-up">
            <div class="mv-icon">🎯</div>
            <div>
                <h4 style="color:#fff;margin-bottom:8px;">Mission</h4>
                <p style="color:#aaa;font-size:14px;line-height:1.8;">To engineer, rebuild, and manufacture critical heavy-machine systems delivering validated solutions that restore performance and extend lifecycle across mining, ports, and construction.</p>
            </div>
        </div>
        <div class="mv-card" data-aos="fade-up" data-aos-delay="150">
            <div class="mv-icon">👁️</div>
            <div>
                <h4 style="color:#fff;margin-bottom:8px;">Vision</h4>
                <p style="color:#aaa;font-size:14px;line-height:1.8;">A world where heavy industry operates with confidence enabled by accessible engineering expertise and accountable performance.</p>
            </div>
        </div>
    </div>
</section>

<!-- TRUST. TENACITY. TOGETHERNESS. -->
<div class="trust-hero" data-aos="fade-in">
    <h2>Trust.<br>Tenacity.<br>Togetherness.</h2>
    <div class="trust-text">
        <div>
            <p>At BSAS, our values are not abstract ideals. They define how we engineer, rebuild, and manufacture every solution we deliver.</p>
            <p>Rooted in Indian ethos and inspired by <strong>Vasudhaiva Kutumbakam</strong> — the belief that the world is one family — we see our customers as long-term partners, not transactions.</p>
        </div>
        <div>
            <p><strong>Trust</strong> is built over journeys, not promises.</p>
            <p><strong>Tenacity</strong> is demonstrated through performance under pressure.</p>
            <p><strong>Togetherness</strong> is forged when we stand shoulder-to-shoulder with our partners on site.</p>
        </div>
    </div>
</div>

<!-- TEAM -->
<section class="section section-grey">
    <div class="section-label">The People</div>
    <h2 class="section-title text-center mb-40" style="color:var(--orange);">Team</h2>
    <div class="team-grid">
        <div class="team-card" data-aos="fade-up">
            <div class="team-photo"></div>
            <div class="team-name">Mr. Soumyadipto Monda</div>
            <div class="team-role">CRM</div>
            <span class="team-dept">Sales</span>
        </div>
        <div class="team-card" data-aos="fade-up" data-aos-delay="100">
            <div class="team-photo"></div>
            <div class="team-name">Mr. Devi Chaudhary</div>
            <div class="team-role">Sales</div>
            <span class="team-dept" style="background:#e33;">Sales</span>
        </div>
        <div class="team-card" data-aos="fade-up" data-aos-delay="200">
            <div class="team-photo"></div>
            <div class="team-name">Mr. Arjun Pal</div>
            <div class="team-role">Purchase</div>
            <span class="team-dept">Purchase</span>
        </div>
        <div class="team-card" data-aos="fade-up" data-aos-delay="300">
            <div class="team-photo"></div>
            <div class="team-name">Mr. Souvik Das</div>
            <div class="team-role">Marketing</div>
            <span class="team-dept">Marketing</span>
        </div>
    </div>
</section>

<!-- VALIDATED -->
<div class="validated-section" data-aos="fade-in">
    <div class="validated-content">
        <h2>Validated.<br>Verified.<br>Accountable.</h2>
        <p>Quality management systems validated to international manufacturing standards.</p>
    </div>
    <div class="validated-img bg-workshop"></div>
</div>
